        ...
        <div class="px-6 py-4 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="请输入企业名称"
            >
            <p class="mt-1 text-xs text-gray-500">用于生成授权书等法律文件</p>
          </div>
...
//   }
//   // 如果已经是完整的URL，直接返回
//   if (form.value.logo.startsWith('http')) {
//     return form.value.logo
//   }
  // 对于相对路径，使用运行时配置的URL前缀进行拼接
  console.log(config.public.fileUrl)
...
const getSystemConfig = async () => {
  try {
    loading.value = true
    const response = await api.post('/admin/system-config/')
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await api.post('/admin/system-config/update/', {
